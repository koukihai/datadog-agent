...
func NewProvider(entityID string) Provider {
	p := &provider{
		...
		forceRefresh:    config.Datadog.GetBool("logs_config.k8s_wait_for_tags"),
		waitOnFirstCall: config.Datadog.GetBool("logs_config.k8s_wait_for_tags"),
	}
	if p.forceRefresh {
		p.forceRefreshDuration = config.Datadog.GetDuration("logs_config.force_tagger_call_duration") * time.Second
		p.taggerWarmupDuration = config.Datadog.GetDuration("logs_config.tagger_warmup_duration") * time.Second
	}
	return p
}
...
// setWaitOnFirstCall udpates the waitOnFirstCall attribute
func (p *provider) setWaitOnFirstCall(b bool) {
	p.mu.Lock()
	defer p.mu.Unlock()
	p.waitOnFirstCall = b
}

// getWaitOnFirstCall returns the waitOnFirstCall attribute
func (p *provider) getWaitOnFirstCall() bool {
	p.mu.Lock()
	defer p.mu.Unlock()
	return p.waitOnFirstCall
}
